...
namespace LembreteDividas
{
    ...
    public partial class Dividas : ContentPage
    {
        ...
        public Dividas()
        {
            ...
            _tituloValor.TituloValor = "Total a pagar (mês 10): R$ 0,00\nTotal a pagar: R$0,00";
            ...
            if (_bills != null)
            {
                ...
                foreach (var item in _bills)
                {
                    ...
                    if (item.DataVencimento.Month == DateTime.Now.Month)
                        valorTotalMes += item.Valor;
                }
                _tituloValor.TituloValor = $"Total a pagar (mês {DateTime.Now.Month}): R${String.Format("{0:F2}", valorTotalMes)}\n" +
                    $"Total a pagar: R${String.Format("{0:F2}", valorTotal)}";
            }
            else
                _tituloValor.TituloValor = $"Total a pagar (mês {DateTime.Now.Month}): R$0,00\n" +
                    $"Total a pagar: R$0,00";
